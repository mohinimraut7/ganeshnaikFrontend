...
export default function BookAppointment() {
    ...
    const fetchCitizenById = async () => {
      try {
        ...
        if (res.data?.success && res.data?.citizen) {
          ...
          if (res.data.lastPhoto) setLastPhotoPreview(res.data.lastPhoto);
          setVisitCount(res.data.nextVisitNumber);
          setCitizenFound(true);
        }
      } catch { } finally { setCitizenLoading(false); }
    };
    fetchCitizenById();
  }, []);
  ...
  useEffect(() => {
    if (step !== 1 || !form.preferredDate || !form.slotTime) {
      if (!form.preferredDate || !form.slotTime) { setMicroSlots([]); setAllSlotsFull(false); }
      return;
    }
    let cancelled = false;
    const capturedDate     = form.preferredDate;
  ...
  if (booked) {
    return (
      <>
        <style>{`
          @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
          .book-root { min-height:calc(100vh - 64px); background:#FFF3E0; display:flex; align-items:center; justify-content:center; padding:32px 16px; font-family:'Plus Jakarta Sans',sans-serif; }
          @keyframes successPop { 0%{transform:scale(0.7);opacity:0} 70%{transform:scale(1.08)} 100%{transform:scale(1);opacity:1} }
          @keyframes fadeUpIn { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:none} }
        `}</style>
        <div className="book-root">
          <div style={{ background:"#fff8f0", borderRadius:20, padding:"48px 40px", maxWidth:520, width:"100%", textAlign:"center", boxShadow:"0 8px 40px rgba(247,128,26,0.18)", border:"1.5px solid #F7801A", animation:"fadeUpIn .5s ease" }}>
            <div style={{ width:90, height:90, borderRadius:"50%", background:"linear-gradient(135deg,#F7801A,#C45000)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:44, margin:"0 auto 20px", animation:"successPop .5s cubic-bezier(.34,1.56,.64,1) both", boxShadow:"0 8px 24px rgba(247,128,26,0.35)" }}>✅</div>
            <h2 style={{ fontSize:26, fontWeight:800, color:"#843A00", marginBottom:8, fontFamily:"'Plus Jakarta Sans',sans-serif", background:"linear-gradient(135deg,#F5731F,#C45000)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Appointment Booked!</h2>
            <p style={{ color:"#C45000", marginBottom:24, fontSize:14, fontWeight:500 }}>तुमची appointment successfully book झाली आहे</p>
            <div style={{ background:"linear-gradient(135deg,#fff8f0,#fff3e0)", border:"1.5px solid #F7801A", borderRadius:12, padding:"20px 24px", marginBottom:24, textAlign:"left" }}>
              {[
                ["Token ID",   booked.tokenId],
                ["Date",       formatShortDate(booked.preferredDate)],
                ["Slot",       booked.slotTime],
                ...
                <div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:"1px solid #ffe0b2", fontSize:13 }}>
                  <span style={{ color:"#C45000", fontWeight:600 }}>{k}</span>
                  <span style={{ color:"#843A00", fontWeight:700 }}>{v}</span>
                </div>
              ))}
            </div>
            {booked.qrCode && (
              <div style={{ marginBottom:20 }}>
                <p style={{ fontSize:12, color:"#C45000", marginBottom:8, fontWeight:600 }}>QR Code — भेटीच्या दिवशी दाखवा</p>
                <img src={booked.qrCode} alt="QR" style={{ width:130, height:130 }} />
              </div>
            )}
            <button onClick={() => navigate("/my-appointments")} style={{ width:"100%", padding:"13px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#F7801A,#C45000)", color:"#fff", fontWeight:700, fontSize:15, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif", boxShadow:"0 4px 16px rgba(247,128,26,0.4)", transition:"transform .15s" }}
              onMouseEnter={e=>e.target.style.transform="translateY(-1px)"}
              onMouseLeave={e=>e.target.style.transform="none"}
            >
              📋 My Appointments बघा
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; }
        ...
        .book-wrapper {
          ...
          background: var(--cream2);
          border-radius: 14px;
          box-shadow: 0 6px 40px rgba(132,58,0,0.15);
          border: 1.5px solid #F7801A;
          overflow: hidden;
        }
        ...
        .book-titlebar {
          ...
          background: linear-gradient(135deg, #F5731F 0%, #C45000 100%);
        }
        .book-titlebar h1 {
          font-size: 22px; font-weight: 800; color: #ffffff;
          margin: 0 0 20px; position: relative; display: inline-block;
        ...
        .book-titlebar h1::after {
          content: ''; position: absolute; bottom: -4px; left: 0;
          width: 48px; height: 3px; background: #FFE0B2; border-radius: 2px;
        }
        ...
        .step-tab.done   { background: rgba(255,255,255,0.15); color: #FFE0B2; }
        .step-tab.active { background: var(--cream); color: var(--or3); border-bottom-color: #FFE0B2; }
        .step-dot {
          width: 22px; height: 22px; border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          font-size: 11px; font-weight: 800; flex-shrink: 0; transition: all .25s;
        }
        .step-dot.done    { background: #843A00; color: #FFE0B2; }
        .step-dot.active  { background: #FFE0B2; color: #843A00; box-shadow: 0 2px 8px rgba(255,224,178,0.5); }
        .step-dot.pending { background: rgba(255,255,255,0.25); color: rgba(255,255,255,0.7); }
        ...
        .f-input:focus { border-color: #C45000; box-shadow: 0 0 0 3px rgba(247,128,26,0.15); background: #fff; }
        .f-input::placeholder { color: #C45000; opacity: 0.5; font-weight: 400; }
        ...
        .date-btn {
          padding: 11px 14px; border-radius: 8px; cursor: pointer; font-weight: 700;
          font-size: 13px; border: 1.5px solid #F7801A; background: #fff8f0;
          color: #843A00; transition: all .18s; text-align: left;
          font-family: 'Noto Sans', 'Mukta', 'Plus Jakarta Sans', sans-serif;
        }
        .date-btn:hover { border-color: #C45000; background: #ffe0b2; color: #6a2e00; transform: translateY(-1px); box-shadow: 0 3px 10px rgba(247,128,26,0.2); }
        .date-btn.sel   { border-color: #C45000; background: #ffe0b2; color: #843A00; box-shadow: 0 3px 10px rgba(247,128,26,0.25); font-weight: 800; }
        .date-btn .sub  { font-size: 11.5px; color: #C45000; font-weight: 500; margin-top: 3px; }

        .slot-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px,1fr)); gap: 10px; }
        .slot-btn {
          padding: 10px 12px; border-radius: 8px; cursor: pointer; font-weight: 700;
          font-size: 12.5px; border: 1.5px solid #F7801A; background: #fff8f0;
          color: #843A00; transition: all .18s;
          font-family: 'Noto Sans', 'Mukta', 'Plus Jakarta Sans', sans-serif;
        }
        .slot-btn:hover { border-color: #C45000; background: #ffe0b2; color: #6a2e00; transform: translateY(-1px); }
        .slot-btn.sel   { border-color: #C45000; background: #ffe0b2; color: #843A00; box-shadow: 0 2px 8px rgba(247,128,26,0.25); font-weight: 800; }

        .info-box { background: #fff3e0; border: 1.5px solid #F7801A; border-radius: 8px; padding: 11px 16px; font-size: 13px; font-weight: 600; color: #843A00; margin-bottom: 16px; }
        .selected-box { background: #fff3e0; border: 1.5px solid #C45000; border-radius: 8px; padding: 11px 16px; font-weight: 700; color: #843A00; font-size: 13px; margin-bottom: 16px; }
        .empty-box { background: linear-gradient(135deg,#fff8f0,#fff3e0); border: 1.5px solid #F7801A; border-radius: 10px; padding: 28px; text-align: center; margin-bottom: 16px; }

        .review-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        @media(max-width:640px){ .review-grid{ grid-template-columns:1fr; } }
        .review-card { background: #fff8f0; border: 1.5px solid #F7801A; border-radius: 10px; padding: 16px 18px; }
        .review-card-title { font-size: 11.5px; font-weight: 800; color: #843A00; text-transform: uppercase; letter-spacing: .8px; margin-bottom: 10px; display: flex; align-items: center; gap: 6px; }
        .review-card-title::before { content:''; display:inline-block; width:14px; height:3px; background:#F7801A; border-radius:2px; }
        .review-row { display:flex; justify-content:space-between; padding:7px 0; border-bottom:1px solid #ffe0b2; font-size:13.5px; }
        .review-row:last-child { border-bottom:none; }
        .review-key { color:#C45000; font-weight:600; }
        .review-val { color:#843A00; font-weight:700; text-align:right; max-width:55%; word-break:break-word; }

        .form-note { font-size:12.5px; color:#C45000; font-weight:500; margin:12px 0 0; }

        .book-nav {
          display: flex; justify-content: flex-end; align-items: center; gap: 12px;
          padding: 24px 40px 32px; border-top: 1.5px solid #F7801A; margin-top: 28px; background: #fff8f0;
        }
        .nav-cancel { padding: 11px 30px; border-radius: 8px; border: 1.5px solid #F7801A; background: #fff8f0; color: #843A00; font-weight: 700; font-size: 13.5px; cursor: pointer; font-family: 'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif; transition: all .18s; }
        .nav-cancel:hover { background:#ffe0b2; border-color:#C45000; color:#6a2e00; }
        .nav-continue { padding: 11px 34px; border-radius: 8px; border: none; background: linear-gradient(135deg,#F7801A,#C45000); color: #fff; font-weight: 800; font-size: 13.5px; cursor: pointer; font-family: 'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif; display: flex; align-items: center; gap: 7px; transition: all .2s; box-shadow: 0 4px 14px rgba(247,128,26,0.4); letter-spacing: 0.2px; }
        .nav-continue:hover:not(:disabled) { background:linear-gradient(135deg,#C45000,#843A00); transform:translateY(-1px); box-shadow:0 6px 18px rgba(196,80,0,0.45); }
        .nav-continue:disabled { background:#e8c8a0; cursor:not-allowed; box-shadow:none; color:#888; }

        .step-progress-bar  { height:4px; background:#ffe0b2; }
        .step-progress-fill { height:100%; background:linear-gradient(90deg,#F7801A,#843A00); transition:width .4s cubic-bezier(.4,0,.2,1); }
        ...
        .photo-btn-primary { padding:11px 24px; border-radius:8px; background:linear-gradient(135deg,#F7801A,#C45000); color:#fff; font-weight:700; font-size:13.5px; cursor:pointer; font-family:'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif; border:none; box-shadow:0 3px 10px rgba(247,128,26,0.3); transition:all .18s; }
        .photo-btn-primary:hover { transform:translateY(-1px); box-shadow:0 5px 14px rgba(247,128,26,0.4); }
        .photo-btn-secondary { padding:11px 24px; border-radius:8px; border:1.5px solid #F7801A; background:#fff8f0; color:#843A00; font-weight:700; font-size:13.5px; cursor:pointer; font-family:'Noto Sans','Mukta','Plus Jakarta Sans',sans-serif; transition:all .18s; }
        .photo-btn-secondary:hover { background:#ffe0b2; }

        .radio-group { display:flex; gap:16px; margin-top:8px; }
        .radio-label { display:flex; align-items:center; gap:8px; cursor:pointer; font-size:14px; font-weight:700; color:#843A00; padding:8px 20px; border-radius:8px; border:1.5px solid #F7801A; background:#fff8f0; transition:all .15s; }
        .radio-label.selected { border-color:#C45000; background:#ffe0b2; color:#6a2e00; }
        .radio-label input { accentColor:#F7801A; width:15px; height:15px; }

        .visit-badge { display:inline-flex; align-items:center; gap:8px; background:linear-gradient(135deg,#C45000,#F7801A); color:#fff; padding:10px 18px; border-radius:10px; font-size:13.5px; font-weight:700; margin-top:4px; box-shadow:0 3px 12px rgba(247,128,26,0.35); letter-spacing:0.2px; }
        .visit-badge .badge-number { background:#843A00; color:#FFE0B2; font-size:15px; font-weight:800; width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 6px rgba(132,58,0,0.4); flex-shrink:0; }

        .last-photo-hint { display:flex; align-items:center; gap:10px; background:#fff3e0; border:1.5px solid #F7801A; border-radius:10px; padding:10px 14px; margin-bottom:14px; font-size:13px; font-weight:600; color:#843A00; }
      `}</style>

      {toast && <div className="toast" style={{ background: toast.type==="success" ? "#F7801A" : "#dc2626" }}>{toast.msg}</div>}
            ...
            <div className={animDir === "forward" ? "step-body-forward" : "step-body-back"} key={step}>
              ...
              {step===0 && (
                <div>
                  ...
                  {citizenLoading && (
                    <div style={{ display:"flex", alignItems:"center", gap:8, color:"#F7801A", fontSize:13, fontWeight:600, marginBottom:16 }}>
                      <span style={{ width:14, height:14, border:"2px solid rgba(247,128,26,0.3)", borderTopColor:"#F7801A", borderRadius:"50%", animation:"spin .7s linear infinite", display:"inline-block" }}/>
                      माहिती लोड होत आहे...
                    </div>
                  )}
              ...
              {step===1 && (
                <div>
                  ...
                  {futureAvail.length === 0 ? (
                    <div className="empty-box">
                      <div style={{ fontSize:36, marginBottom:8 }}>📅</div>
                      <p style={{ color:"#843A00", fontWeight:700, margin:"0 0 4px", fontSize:15 }}>सध्या कोणत्याही dates available नाहीत</p>
                      <p style={{ color:"#C45000", fontSize:13.5, margin:0, fontWeight:500 }}>Admin कडून availability add होण्याची वाट पाहा</p>
                    </div>
                  ) : (
                    <>
                      <div className="field">
                      ...
                      {form.preferredDate && form.slotTime && (
                        <div className="field">
                          <label>15-Minute Appointment Slot निवडा <span className="req">*</span></label>
                          {microLoading ? (
                            <div style={{ color:"#F7801A", fontSize:13, fontWeight:600, padding:"12px 0", display:"flex", alignItems:"center", gap:8 }}>
                              <span style={{ width:14, height:14, border:"2px solid rgba(247,128,26,0.3)", borderTopColor:"#F7801A", borderRadius:"50%", animation:"spin .7s linear infinite", display:"inline-block" }}/>
                              Slots loading...
                            </div>
                          ) : allSlotsFull ? (
                            ...
                            <>
                              ...
                              {microSlots.length > 0 && (
                                <p style={{ fontSize:12, color:"#C45000", marginTop:8, fontWeight:500 }}>
                                  🟢 Available &nbsp;|&nbsp; 🔴 Booked &nbsp;|&nbsp;
                                  <strong>{microSlots.filter(s=>!s.booked).length}</strong> of <strong>{microSlots.length}</strong> slots available
                                </p>
                              )}
                ...
                <div>
                  ...
                  <div className="f-grid-2">
                    ...
                    <div className="field">
                      ...
                      {form.ward && citizenFound && (
                        <p style={{ fontSize:12, color:"#F7801A", fontWeight:600, marginTop:4 }}>✅ मागील भेटीवरून auto-filled</p>
                      )}
                    </div>
                    <div className="field">
                      <label>Have you visited before? <span className="req">*</span></label>
                        ...
                        {["No","Yes"].map(v => (
                          <label key={v} className={`radio-label${form.visitedBefore===(v==="Yes")?" selected":""}`}>
                            <input type="radio" name="vb" value={v}
                              checked={form.visitedBefore===(v==="Yes")}
                              onChange={() => setForm(p=>({...p,visitedBefore:v==="Yes"}))}
                              style={{ accentColor:"#F7801A", width:15, height:15 }} />
                            {v}
                          </label>
                        ))}
                      </div>
                ...
                <div>
                  ...
                  {lastPhotoPreview && !form.photoPreview && (
                    <div className="last-photo-hint">
                      <img src={`${import.meta.env.VITE_BASE_URL || "http://localhost:4200"}/${lastPhotoPreview}`}
                        alt="last visit"
                        style={{ width:48, height:48, borderRadius:"50%", objectFit:"cover", border:"2px solid #F7801A", flexShrink:0 }}
                        onError={e => { e.target.style.display="none"; }}
                      />
                      <div>
                        <p style={{ margin:0, fontWeight:700, fontSize:13 }}>मागील भेटीचा फोटो उपलब्ध आहे</p>
                        <button type="button"
                          style={{ marginTop:4, fontSize:12, color:"#F7801A", fontWeight:700, background:"none", border:"none", cursor:"pointer", padding:0, textDecoration:"underline" }}
                          onClick={() => {
                            setForm(p => ({ ...p, photoPreview: `${import.meta.env.VITE_BASE_URL || "http://localhost:4200"}/${lastPhotoPreview}`, visitorPhoto: lastPhotoPreview }));
                          }}>
                          हाच फोटो वापरा ↗
                  ...
                  {showCam ? (
                    <div style={{ position:"relative", borderRadius:12, overflow:"hidden", border:"2px solid #F7801A", marginBottom:16 }}>
                      <button type="button" onClick={stopCam} style={{ position:"absolute", top:10, right:10, zIndex:10, width:30, height:30, borderRadius:"50%", background:"#ef4444", color:"#fff", border:"none", cursor:"pointer", fontWeight:800 }}>✕</button>
                      <video ref={videoRef} autoPlay playsInline style={{ width:"100%", maxHeight:340, objectFit:"cover", display:"block" }} />
                      <canvas ref={canvasRef} style={{ display:"none" }} />
                      <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:16, background:"linear-gradient(transparent,rgba(0,0,0,0.55))", display:"flex", justifyContent:"center" }}>
                        <button type="button" onClick={capturePic} style={{ padding:"11px 30px", borderRadius:9, border:"none", background:"linear-gradient(135deg,#F7801A,#C45000)", color:"#fff", fontWeight:800, fontSize:14, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif", boxShadow:"0 4px 16px rgba(247,128,26,0.4)" }}>📸 Capture Photo</button>
                      </div>
                    </div>
                  ) : (
                    <div style={{ border:"2.5px dashed #F7801A", borderRadius:14, padding:36, textAlign:"center", background:"linear-gradient(135deg,#fff8f0,#fff3e0)", marginBottom:16, transition:"all .2s" }}>
                      {form.photoPreview ? (
                        <>
                          <img src={form.photoPreview} alt="preview" style={{ width:110, height:110, borderRadius:"50%", objectFit:"cover", border:"3px solid #F7801A", marginBottom:14, boxShadow:"0 4px 20px rgba(247,128,26,0.3)" }} />
                          <p style={{ color:"#843A00", fontWeight:700, fontSize:14, margin:"0 0 4px" }}>✅ Photo selected</p>
                          <p style={{ color:"#C45000", fontSize:12.5, margin:0, fontWeight:500 }}>Click "Upload Photo" to change</p>
                        </>
                      ) : (
                        <>
                          <div style={{ fontSize:44, marginBottom:12, color:"#F7801A" }}>📷</div>
                          <p style={{ color:"#C45000", fontSize:14, margin:0, fontWeight:600 }}>Upload किंवा camera वापरून photo द्या</p>
                        </>
                      )}
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div className="review-grid">
                    ...
                    {form.photoPreview && (
                      <div className="review-card" style={{ textAlign:"center" }}>
                        <div className="review-card-title">Visitor Photo</div>
                        <img src={form.photoPreview} alt="v" style={{ width:88, height:88, borderRadius:"50%", objectFit:"cover", border:"3px solid #F7801A", marginTop:8, boxShadow:"0 4px 16px rgba(247,128,26,0.25)" }} />
                      </div>
                    )}
                  </div>
                  <div style={{ background:"linear-gradient(135deg,#fff8f0,#fff3e0)", border:"1.5px solid #F7801A", borderRadius:10, padding:"13px 18px", margin:"16px 0 0", fontSize:13.5, color:"#843A00", fontWeight:600 }}>
                    ⚠️ Submit केल्यानंतर Admin approval नंतर appointment confirm होईल.
                  </div>
                </div>
              )}
